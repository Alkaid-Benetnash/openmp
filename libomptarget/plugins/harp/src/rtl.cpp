...
#define HARP_ERR_STRING(err)                                               \
  ...
  #define TIME_PRINT(x)  t_end = rtclock(); printf("[time][harp] %s = %0.6lfs\n", x, t_end - t_start);
  double rtclock() {
    struct timezone Tzp;
    struct timeval Tp;
    int stat;
...
int32_t __tgt_rtl_set_module(void* module) {
  ...
  DP("[harp] __tgt_rtl_set_module success\n");

  return OFFLOAD_SUCCESS;
}

int32_t __tgt_rtl_is_valid_binary(__tgt_device_image *image) {
  uint32_t is_valid_binary = elf_check_machine(image, EM_X86_64, 9002);

  DP("[harp] __tgt_rtl_is_valid_binary\n");
...
int32_t __tgt_rtl_data_retrieve(int32_t device_id, void *hst_ptr, void *tgt_ptr,
  ...
  DP("[harp] __tgt_rtl_data_retrieve\n");
...
int32_t __tgt_rtl_run_target_team_region(int32_t device_id, void *tgt_entry_ptr,
  ...
  DP("[harp] __tgt_rtl_run_target_team_region\n");
  TIME_START
...
int32_t __tgt_rtl_run_target_region(int32_t device_id, void *tgt_entry_ptr,
  ...
  DP("[harp] __tgt_rtl_run_target_region\n");
